...
public class CorridorFirstDungeonGeneration : SimpleRandomWalkDungeonGenerator
{
    ...
    private List<Vector2Int> FindAllDeadEnds(HashSet<Vector2Int> floorPositions)
    {
        ...
        foreach (var positon in floorPositions)
        {
            ...
            {
                if(floorPositions.Contains(positon + direction))
                {
                    neighboursCount++;
                }
                if(neighboursCount == 1)
                {
                    deadEnds.Add(positon);
                }
            }
        }
        return deadEnds;
    ...
    private void CreateCorridors(HashSet<Vector2Int> floorPositions, HashSet<Vector2Int> potentialRoomPositions)
    {
        ...
        {
            var corridor = ProceduralGenerationsAlgorythm.RandomWalkCorridor(currentPosition, corridorLength);
